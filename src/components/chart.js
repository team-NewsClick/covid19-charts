...
import {
  XYPlot,
  LineSeries, Crosshair, MarkSeries, LabelSeries,
...
export default function LineChart({ covidData }) {
  ...
  const [multiRegions, setMultiRegions] = useState([{value: '144', label: 'India'}])
  ...
  return (
    <div>
      ...
      >
        ...
        <Crosshair
          values={crosshair}
          titleFormat={(d) => ({title: 'Day ' + (d[0].x), value: _dayDate(d[0].x)})}
        ...
        {multiRegions.map(({value, label}, index) =>
          <MarkSeries
              key={value}
              strokeWidth={2}
              data={[{x: (covidData.data.length-1)/267, y: covidData.data[(covidData.data.length-1)-(267-value+1)].total_confirmed, label: label}]}
              color={customColor[index]}
          />
        )}
        {multiRegions.map(({value, label}) =>
          <LabelSeries
              key={value}
              data={[{x: (covidData.data.length-1)/267, y: covidData.data[(covidData.data.length-1)-(267-value+1)].total_confirmed, label: label, xOffset: 12}]}
